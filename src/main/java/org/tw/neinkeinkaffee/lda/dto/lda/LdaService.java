...
import org.tw.neinkeinkaffee.lda.dto.document.SectionRepository;
...
public class LdaService {
    ...
    public void save(DtoLda dtoLda) {
        ...
        List<DtoDocument.DtoSection> sections = groupBySectionAndVolume(dtoLda.getDocuments());
        for (DtoDocument.DtoSection section : sections) {
    ...
    public List<DtoDocument.DtoSection> fetchAllByAndGroupBySections(String corpusName, int numberOfTopics, String timestamp) {
    ...
    List<DtoDocument.DtoSection> groupBySectionAndVolume(List<DtoDocument> documents) {
        HashMap<String, List<DtoDocument>> volumes = new HashMap<>(documents.stream()
            .collect(groupingBy(DtoDocument::getVolume)));
        HashMap<String, List<DtoDocument.DtoVolume>> sections = new HashMap<>(volumes.entrySet().stream()
            .map(entry -> DtoDocument.DtoVolume.builder()
                .title(entry.getKey())
        ...
        List<DtoDocument.DtoSection> sectionsList = sections.entrySet().stream()
            .map(entry -> {
                List<DtoDocument.DtoVolume> volumesList = entry.getValue();
                Collections.sort(volumesList);
                return DtoDocument.DtoSection.builder()
                    .title(entry.getKey())
                    .volumes(volumesList)
                    .build();
            })
